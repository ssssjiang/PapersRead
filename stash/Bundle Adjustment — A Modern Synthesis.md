...
- 误差建模：robust statistically-based error metrics based on total (inlier + outlier) log likelihoods should be used.
  ...
  - 成本函数还要考虑到outlier
    ...
    > #K-L散度
    >
    > ​		在概率学和统计学上，我们经常会使用一种`更简单的、近似的分布`来替代`观察数据`或`太复杂的分布`。K-L散度能帮助我们度量使用一个分布来近似另一个分布时所损失的信息量。
    >
    ...
    - 计算的负对数似然成本函数：$-\sum_{i} \log p_{\text {model }}\left(\mathbf{z}_{i} \mid \mathbf{x}\right)$，其实和交叉熵一致：$-n_{\mathbf{z}} \int p_{0}(\mathbf{z}) \log \left(p_{\text {model }}(\mathbf{z} \mid \mathbf{x})\right) \mathrm{dz}$，再加上常量$p_{0}(\mathbf{z})$，和相对熵（KL散度）一致：$\int p_{0}(\mathbf{z}) \log \left(p_{0}(\mathbf{z}) / p_{\text {model }}(\mathbf{z} \mid \mathbf{x})\right) d \mathbf{z}$，因此，`ML 估计逐渐收敛到预测观测分布具有最小相对熵的模型`。
    - ML模型对outlier非常敏感，outlier可能对相对熵有非常大的贡献，因此，我们设计的误差分布，最好使得outlier不会有极大的$-\log p_{\text {model }}$。
      - 
